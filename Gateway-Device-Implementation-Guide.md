...
1. 接收圖像數據並保存到設備
2. 更新本地存儲的 `imageCode`
3. 下次 `deviceStatus` 回報時包含新的 `imageCode`
...
class ImageProcessor:
    ...
    async def handle_image_update(self, device_mac, image_data, image_code):
        """處理圖像更新消息"""
        # 1. 保存圖像數據
        # 2. 更新本地 imageCode
        # 3. 發送圖像到設備
...
2. **架構設計**：Gateway 和 Device 的內部架構建議
3. **錯誤處理**：完善的錯誤處理和恢復機制
4. **安全考量**：重要的安全注意事項
5. **實作範例**：可直接使用的代碼框架
